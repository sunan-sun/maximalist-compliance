...
=================================================
Uses the identified base parameters π̂_b to compute inverse dynamics
torques that cancel the robot's nonlinear dynamics, then applies
linear feedback to track a desired trajectory.

Control law:
    q̈_cmd = q̈_d + Kd·(q̇_d − q̇) + Kp·(q_d − q)
    τ     = Y_b(q, q̇, q̈_cmd) · π̂_b

This is equivalent to:
    τ = M̂(q)·q̈_cmd + Ĉ(q,q̇)·q̇ + Ĝ(q)

but computed in a single regressor evaluation — no need to reconstruct
M, C, G separately.

Closed-loop error dynamics (with perfect identification):
    ë + Kd·ė + Kp·e = 0

Usage:
    python computed_torque_control.py
    python computed_torque_control.py --gui
    python computed_torque_control.py --Kp 100 --Kd 20
"""
...
def figure_eight_trajectory(t, nv, omega=0.8, amplitude=0.4):
    ...
    Each joint gets a different phase offset so the motion is visually
    interesting and exercises multiple joints simultaneously.
    """
    q_d   = np.zeros(nv)
    dq_d  = np.zeros(nv)
    ddq_d = np.zeros(nv)
    ...
    for j in range(nv):
        phase = j * np.pi / nv
        amp = amplitude * (1.0 - 0.2 * j / max(nv - 1, 1))  # slightly less for distal joints
...
# ─────────────────────────────────────────────────────────────────────────────
# Regressor-based inverse dynamics (single time step)
# ─────────────────────────────────────────────────────────────────────────────
...
def regressor_inverse_dynamics(pin_model, pin_data, q, dq, ddq_cmd, bp, pi_b):
    ...
    Single regressor evaluation gives the full inverse dynamics torque.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Simulation (Pinocchio forward dynamics, no MuJoCo dependency)
# ─────────────────────────────────────────────────────────────────────────────
...
def simulate_pinocchio(pin_model, pin_data, bp, pi_b_est, pi_b_true,
    ...
    Runs two controllers in parallel:
      - CTC with identified parameters (π̂_b)
      - PD + gravity compensation (baseline)
    """
    nv = pin_model.nv

    # PD gains: scale per joint by inertia (same as momentum_observer)
    ...
    for i, t in enumerate(t_arr):
        ...
        # ── CTC: computed torque control with identified parameters ─────────
        e_ctc    = q_d - q_ctc_r
        de_ctc   = dq_d - dq_ctc
...
def plot_comparison(log, robot_name):
    ...
    fig.suptitle(
        ...
        f"CTC uses identified regressor · π̂_b  |  PD+G uses per-joint gains + gravity comp",
        fontsize=11, fontweight="bold",
    )

    # ── Top-left: CTC joint tracking ───────────────────────────────────────
    ax = axes[0, 0]
    for j in range(nv):
    ...
    ax = axes[1, 0]
    e_ctc_rms = np.sqrt(np.mean(log["e_ctc"]**2, axis=0))
    e_pd_rms  = np.sqrt(np.mean(log["e_pd"]**2, axis=0))
    for j in range(nv):
        c = colors[j % len(colors)]
    ...
    # ── Bottom-right: torque comparison ─────────────────────────────────────
    ax = axes[1, 1]
    for j in range(nv):
        c = colors[j % len(colors)]
    ...
    ax.set_title("Control torques  τ")
    ax.set_ylabel("N·m"); ax.set_xlabel("t [s]")
    ax.legend(fontsize=5, ncol=2); ax.grid(True, alpha=0.3)

    plt.tight_layout()
    out = f"computed_torque_{robot_name}.png"
...
def plot_mujoco(log, robot_name):
    ...
    plt.tight_layout()
    out = f"computed_torque_mujoco_{robot_name}.png"
    plt.savefig(out, dpi=150)
    print(f"\n  Plot saved: {out}")
    plt.show()
...
if __name__ == "__main__":
    ...
    pi_b_est, pi_b_true, bp = ident.run(T=args.T_id, plot=False)
